...
exports.getStudyStats = exports.updateStudySession = exports.getStudySessionById = exports.getStudySessions = exports.createStudySession = void 0;
...
const createStudySession = async (req, res) => {
    try {
        ...
        const flashcards = await flashcard_model_1.default.find({
            _id: { $in: flashcardIds },
            user: req.userId
        });
        ...
        // Create a new study session
        const studySession = new studySession_model_1.default({
            user: req.userId,
...
const getStudySessions = async (req, res) => {
    try {
        ...
        // Get study sessions
        const studySessions = await studySession_model_1.default.find({ user: req.userId })
            .sort({ startTime: -1 })
            .skip(skip)
            .limit(Number(limit))
            .populate('flashcards', 'question answer');
        // Get total count for pagination
        const total = await studySession_model_1.default.countDocuments({ user: req.userId });
        res.json({
            studySessions,
            pagination: {
                total,
...
const getStudySessionById = async (req, res) => {
    try {
        const studySession = await studySession_model_1.default.findOne({
            _id: req.params.id,
            user: req.userId
...
exports.getStudySessionById = getStudySessionById;
...
const updateStudySession = async (req, res) => {
    try {
        ...
        if (results && Array.isArray(results)) {
            ...
            const allResultsValid = results.every((result) => validFlashcardIds.includes(result.flashcard.toString()));
            if (!allResultsValid) {
            ...
            results.forEach((result) => {
                if (result.result === studySession_model_1.ReviewResult.CORRECT) {
                    correctCount++;
                }
                else if (result.result === studySession_model_1.ReviewResult.INCORRECT) {
                    incorrectCount++;
                }
            });
            ...
            await Promise.all(results.map(async (result) => {
                ...
                await flashcard_model_1.default.updateOne({ _id: result.flashcard, user: req.userId }, updateData);
            }));
        }
...
const getStudyStats = async (req, res) => {
    try {
        ...
        for (let i = Number(days) - 1; i >= 0; i--) {
            ...
            const daySessions = sessions.filter(session => session.startTime >= date && session.startTime < nextDate);
